...
const isSetResult = computed(
  () => owners.value?.length != 0 || years.value?.length != 0 || weeks.value?.length != 0,
)
...
const weeks = computedAsync(async () => {
  const list = Array.from({ length: 52 }, (_, index) => {
    const week = 1 + index
    return {
      id: week,
      name: week.toString(),
  ...
  <div class="flex-1/2">
    ...
    >
      <div class="flex flex-col-5 gap-10 mb-2 mx-2">
        ...
        <SearchBlock @click="select" id="week" label="week" :items="weeks"></SearchBlock>
