...
<template>
  <div v-if="isOpen" class="modal-mask">
    <div class="modal-wrapper">
      <div
        ref="target"
        class="relative m-auto my-50 p-4 w-2/5 min-w-[30%] max-w-[30%] rounded-lg bg-white shadow-sm"
      >
        <div class="flex items-start justify-between p-2">
          <div>
            <h5 class="text-gray-700 text-xl font-bold">Change of driver and vehicle</h5>
          </div>
          <button
            @click="() => emit('modal-close-2')"
        ...
        <div class="grid grid-cols-1 place-items-end px-3">
          <button
            @click="() => emit('modal-close-2')"
